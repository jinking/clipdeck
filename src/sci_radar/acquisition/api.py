...
from sci_radar.acquisition.domain import AcquisitionInput, BlobRole, SourceKind
...
async def submit_file(
    ...
    fd, temporary = tempfile.mkstemp(prefix="sci-upload-")
    size = 0
    try:
...
async def get_blob(blob_id: str, request: Request):
    ...
    from sci_radar.acquisition.storage import extension_for_blob
...
async def health(request: Request):
    ...
    payload = {"status": "ok" if healthy else "degraded", "service": "sci-acquisition", "checks": checks}
